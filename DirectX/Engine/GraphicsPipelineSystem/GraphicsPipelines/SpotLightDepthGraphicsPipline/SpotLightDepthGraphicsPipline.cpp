#include "SpotLightDepthGraphicsPipline.h"
#include "Engine/Base/DebugLog/DebugLog.h"
#include <cassert>

SpotLightDepthGraphicsPipline::SpotLightDepthGraphicsPipline()
{
	Initialize();
}

void SpotLightDepthGraphicsPipline::InitializePSO()
{
	//RootSignature作成
	...
	pixelShaderBlob_ = CompileShader(L"Resources/Shaders/GraphicsShaders/SpotLightShader/SpotLightDepth.PS.hlsl", L"ps_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
	...
	// 比較関数をLessEqualに設定し、手前であれば深度を書き込む
